...
class DeviceTier(str, Enum):
    """设备虚拟层级枚举（基于能力评分）"""
    T1_HIGH_PERFORMANCE = "T1"  # 高性能设备：综合评分 >= 1500
    T2_MEDIUM_PERFORMANCE = "T2" # 中性能设备：综合评分 500-1499
    T3_LOW_PERFORMANCE = "T3"    # 低性能设备：综合评分 < 500
...
class DeviceInfo:
    ...
    def get_total_score(self) -> int:
        ...
        for cap in DeviceCapability:
            total += self.get_capability_score(cap)
        return total

    def get_tier(self) -> DeviceTier:
        """获取设备虚拟层级"""
        score = self.get_total_score()
        if score >= 1500:
            return DeviceTier.T1_HIGH_PERFORMANCE
        elif score >= 500:
            return DeviceTier.T2_MEDIUM_PERFORMANCE
        else:
            return DeviceTier.T3_LOW_PERFORMANCE

    def get_tier_name(self) -> str:
        """获取设备层级名称（中文）"""
        tier_names = {
            DeviceTier.T1_HIGH_PERFORMANCE: "高性能(T1)",
            DeviceTier.T2_MEDIUM_PERFORMANCE: "中性能(T2)",
            DeviceTier.T3_LOW_PERFORMANCE: "低性能(T3)"
        }
        return tier_names.get(self.get_tier(), "未知")
